  ...
  useEffect(() => {
    const fetchLikedMovies = async () => {
      ...
      try {
        ...
        const response = await fetch(`${baseUrl}/liked`, {
          method: "GET",
          credentials: "include",
        });
        ...
        const likedItems = data.items || [];

        // Fetch full movie details from backend for each liked item
        const moviePromises = likedItems.map(async (item) => {
      ...
      <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400">
            You need to be logged in to see your liked movies.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold mb-8">Your Liked Movies</h1>

        {likedMovies.length === 0 ? (
          <div className="text-center py-12">
            <h2 className="text-xl font-semibold mb-4">No liked movies yet</h2>
            <p className="text-gray-400">
